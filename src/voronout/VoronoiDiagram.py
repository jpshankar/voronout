...
class VoronoiDiagram:
    def __init__(self, basePoints: tuple[Point], planeWidth: float, planeHeight: float):
        ...
        self._spatialSiteRegionNeighbors: dict[uuid4, dict[uuid4, uuid4]] = {}
        ...
        for ([edgeVertex0Index, edgeVertex1Index], [edgeSite0Index, edgeSite1Index]) in edgeVerticesAndSites:
            ...
            # Add edge info to both regions containing the edge. edgeLength should only be calculated when neither vertex is None.
            site0Edge = VoronoiEdge(edgeId = uuid4(), vertex0Id = edgeVertex0Id, vertex1Id = edgeVertex1Id, edgeLength = None)
            
            voronoiRegionsInfo[edgeSite0Index].addEdge(edge = site0Edge)        
            voronoiRegionsInfo[edgeSite0Index].addNeighborRegionId(borderingEdgeId = site0Edge.edgeId, neighborRegionId = edgeSite1Id)

            site1Edge = VoronoiEdge(edgeId = uuid4(), vertex0Id = edgeVertex0Id, vertex1Id = edgeVertex1Id, edgeLength = None)
            
            voronoiRegionsInfo[edgeSite1Index].addEdge(edge = site1Edge)
            voronoiRegionsInfo[edgeSite1Index].addNeighborRegionId(borderingEdgeId = site1Edge.edgeId, neighborRegionId = edgeSite0Id)
        ...
        for voronoiRegionInfo in voronoiRegionsInfo:
            ...
            for voronoiRegionEdge in voronoiRegionInfo.edges():
                ...
                edgeVertex0 = None if not edgeVertex0Id else self._spatialDiagramVertices[edgeVertex0Id]
                edgeVertex1 = None if not edgeVertex1Id else self._spatialDiagramVertices[edgeVertex1Id]

                vertex0OutsideDiagram = not edgeVertex0
                vertex1OutsideDiagram = not edgeVertex1
                ...
                if not (vertex0OutsideDiagram or vertex1OutsideDiagram):
                    ...
                    # Quick workaround for 0-length edges - refine later.
                    if latestEdgeVertex0Id != latestEdgeVertex1Id:
                        latestEdge = VoronoiEdge(edgeId = voronoiRegionEdge.edgeId, vertex0Id = latestEdgeVertex0Id, vertex1Id = latestEdgeVertex1Id, edgeLength = Point.distance(p1 = latestEdgeVertex0, p2 = latestEdgeVertex1))
                    ...
                    neighborSiteId = voronoiRegionInfo.getEdgeNeighborRegion(edgeId = voronoiRegionEdge.edgeId)
                    ...
                    if boundaryIntersectionPointId != latestPointInsideDiagramId:
                        latestEdgeLength = Point.distance(p1 = latestPointInsideDiagram, p2 = latestBoundaryIntersection)
                        latestEdge = VoronoiEdge(edgeId = voronoiRegionEdge.edgeId, vertex0Id = boundaryIntersectionPointId, vertex1Id = latestPointInsideDiagramId, edgeLength = latestEdgeLength) if vertex0OutsideDiagram else VoronoiEdge(edgeId = uuid4(), edgeVertex0Id = latestPointInsideDiagramId, edgeVertex1Id = boundaryIntersectionPointId, edgeLength = latestEdgeLength)
                        self._spatialSiteRegionBoundaries[regionId].append(latestEdge)
                else:
            ...
            self._spatialSiteRegionNeighbors[regionId] = voronoiRegionInfo.edgesToNeighbors()
        ...
        self.diagramEdges = []
        for voronoiRegion in self.voronoiRegions.values():
            for voronoiRegionEdge in voronoiRegion.edges:
               self.diagramEdges.append(voronoiRegionEdge)
